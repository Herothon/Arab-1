...
class UploadSender:
    ...
    async def _next(self, data: bytes) -> None:
        ...
        log.debug(
            f"ارسال الملف : {self.request.file_part}/{self.part_count}"
            f"  {len(data)} بايت "
        )
...
class ParallelTransferrer:
    ...
    async def _create_sender(self) -> MTProtoSender:
        ...
        if not self.auth_key:
            log.debug(f"تصدير مصادقة تخزين هيثون : {self.dc_id}")
    ...
    async def download(
        ...
        while part < part_count:
            ...
            for task in tasks:
                ...
                yield data
                part += 1
                log.debug(f"جزء {part} تم تنزيله")

        log.debug("انتهى التنزيل  ، وتنظيف الاتصالات")
        await self._cleanup()
