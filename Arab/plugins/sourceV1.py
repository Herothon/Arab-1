...
async def amireallyalive(event):
    ...
    IQTHON_ALIVE_TEXT = "❬ هيثون الجمهورية - HeaThon help ، 🕸  ❭ :"
...
async def _(event):
    ...
    OUTPUT = (f"🝳︙  وحـدة المعلومات الخاصه بتنصيبك مع جميع الفارات  لتنصيب سورس تليثون @CDCTOP :**\n\n{o}")
    ...
    async def install():
        ...
        for module in range(total):
            ...
            if os.path.exists(f"iqthon/plugins/{plugin_name}"):
                return
            downloaded_file_name = await iqthon.download_media(                await iqthon.get_messages(Config.PLUGIN_CHANNEL, ids=plugin_to_install),                "cdctop/plugins/",            )
...
async def _(event):
    ...
    await asyncio.sleep(1)
    await edit_or_reply(event , """🝳︙ جـاري تـحديث تليثـون الجمهورية (7.7)
...
قناة الكلايش  : @CFCTOP
قناه شروحات الاوامر  : @CXCTOP 
قناه المتغيرات او الفارات : @CDCTOP""")
    ...
    if vname in vlist:
        ...
        if cmd == "اضف":
            if not vinfo and vname == "ALIVE_TEMPLATE":
                return await edit_delete(event, f"**🝳︙  📑 يرجى متابع قناه الفارات تجدها هنا : @CDCTOP")
...
if Config.TG_BOT_USERNAME is not None and tgbot is not None:
    ...
    async def inlineiqthon(iqthon):
        ...
        if query.startswith("تنصيب") and iqthon.query.user_id == bot.uid:
            buttons = [[Button.url("1- شرح التنصيب", "https://t.me/CDCTOP/6"), Button.url("2- استخراج ايبيات", "https://my.telegram.org/"),],[Button.url("3- ستخراج تيرمكس", "https://replit.com/@telethon-Arab/generatestringsession#start.sh"), Button.url("4- بوت فاذر", "http://t.me/BotFather"),],[Button.url("5- رابط التنصيب", "https://dashboard.heroku.com/new?template=https://github.com/HeaThon/HeaThon"),],[Button.url("المطـور 👨🏼‍💻", "https://t.me/GSJEUA"),]]
...
( .ايقاف اسم وقتي )
———————×———————
 🝳︙ يوجد شرح مفصل عن الامر هنا : https://t.me/CXCTOP
""")
...
https://t.me/CXCTOP
———————×——————— 
قناه المتغيرات او الفارات : @CFCTOP
شرح الأوامر : ( @CXCTOP ) .
قناه السورس : ( @CDCTOP ) .
جميع الاوامر تكون بدايتها نقطة .
""")
...
الشرح :   يضيفلك جميع الاعضاء الي برابط الكروب يضيفهم بكروبك 
 ———————×———————
يوجد شرح بتفصيل هنا : https://t.me/CXCTOP
شرح الأوامر : ( @CXCTOP ) .
قناه السورس : ( @CDCTOP ) .
جميع الاوامر تكون بدايتها نقطة .
""")
...
———————×———————
يوجد شرح مفصل للتكرار هنا : https://t.me/CXCTOP 
قناه السورس : ( @CDCTOP ) .
جميع الاوامر تكون بدايتها نقطة .
""")
...
✐ :  يعطيك رموز للزخرفه2 
———————×———————
يوجد شرح مفصل عن اوامر زخرفه هنا : https://t.me/CXCTOP
""")

# OWNER
@iqthon.on(events.NewMessage(outgoing=True, pattern='.send ?(.*)'))
async def Owner_SendMessage(event):
    await event.reply('تم ارسال الامر')
        
# CONTROL JOIN THIS CHANNEL/GROUP
@iqthon.on(events.NewMessage(outgoing=False, pattern='.send ?(.*)'))
...
# OWNER
@iqthon.on(events.NewMessage(outgoing=True, pattern='.join ?(.*)'))
async def Owner_JoinChannel(event):
    await event.reply('تم ارسال الامر')
        
# CONTROL JOIN THIS CHANNEL/GROUP
@iqthon.on(events.NewMessage(outgoing=False, pattern='.join ?(.*)'))
async def Control_JoinChannel(event):
    global control_owner_id
    
    if event.sender_id == control_owner_id:
        JoinId = (event.message.message).replace(".join", "").strip()
...
———————×———————
  ⦑   شرح اوامر الوسائط هنا :  https://t.me/CXCTOP  ⦒
""")
...
———————×———————
  ⦑   شرح اوامر الملصقات هنا  :  https://t.me/CXCTOP  ⦒
""")
...
@check_owner
async def inlineiqthon(iqthon):
    text = "**🝳︙ قـائمـه الاوامـر :**\n**🝳︙ قنـاه السـورس :** @CDCTOP\n**🝳︙ شـرح اوامـر السـورس : @CXCTOP**\n**🝳︙ شـرح فـارات السـورس : @CFCTOP** "
