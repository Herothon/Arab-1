...
async def amireallyalive(event):
    ...
    IQTHON_ALIVE_TEXT = "❬ هيـثون العـرب - Heathon-Arabe ، 🕸  ❭ :"
...
async def _(event):
    ...
    OUTPUT = (f"♛︙وحـدة المعلومات الخاصه بتنصيبك مع جميع الفارات  لتنصيب سورس هيثون @CDCTOP :**\n\n{o}")
...
async def _(event):
    ...
    await asyncio.sleep(1)
    await edit_or_reply(event , "حسنا جاري تحديث هيثون العرب انتضر من 5 الى 10 دقائق")
    try:
...
async def variable(var):
    ...
    if exe == "get":
        ...
        if variable in heroku_var:
            await ics.edit("**⌔ تم تغيـر** `{}` **:**\n **- المتغير :** `{}` \n**- يتم الان اعـادة تشغيـل بـوت هيثـون يستغـرق الامر 2-1 دقيقـه ▬▭ ...**".format(variable, value))
        else:
...
if Config.TG_BOT_USERNAME is not None and tgbot is not None:
    ...
    async def inlineiqthon(iqthon):
        ...
        if query.startswith("تنصيب") and iqthon.query.user_id == bot.uid:
            buttons = [[Button.url("1- شرح التنصيب", "https://t.me/CDCTOP/6"), Button.url("2- استخراج ايبيات", "https://my.telegram.org/"),],[Button.url("3- ستخراج تيرمكس", "https://replit.com/@cdctop/REPUBLIC-OF-GYPSIES-1?v=1"), Button.url("4- بوت فاذر", "http://t.me/BotFather"),],[Button.url("5- رابط التنصيب", "https://dashboard.heroku.com/new?template=https://github.com/HeaThon/HeaThon"),],[Button.url("المطـور 👨🏼‍💻", "http://t.me/GSJEUA"),]]
...
( .ايقاف اسم وقتي )
———————×———————
 ♛︙ يوجد شرح مفصل عن الامر هنا : @CVCTOP
""")
...
https://t.me/CFCTOP/2
———————×——————— 
قناه المتغيرات او الفارات : @CVCTOP
شرح الأوامر : ( @CFCTOP ) .
قناه السورس : ( @CDCTOP ) .
جميع الاوامر تكون بدايتها نقطة .
""")
...
———————×———————
يوجد شرح مفصل للتكرار هنا : @CVCTOP 
قناه السورس : ( @CDCTOP ) .
جميع الاوامر تكون بدايتها نقطة .
""")
...
✐ :  يعطيك رموز للزخرفه2 
———————×———————
يوجد شرح مفصل عن اوامر زخرفه هنا : @CVCTOP
""")
...
@check_owner
async def inlineiqthon(iqthon):
    text = "**♛︙ قـائمـه الاوامـر :**\n**♛︙ قنـاه السـورس :** @CDCTOP\n**♛︙ شـرح اوامـر السـورس : @CVCTOP**\n**♛︙ شـرح فـارات السـورس : @CGCTOP** "
